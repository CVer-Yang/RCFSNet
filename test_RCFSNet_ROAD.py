...
def test_ce_net_vessel():
    source = './dataset/ROAD/test/images/'
    val = os.listdir(source)
    disc = 20
    solver = TTAFrame(RCFSNet)
    #加载训练模型权重
    solver.load('./weights/lunwen3_12plus4_2.th')

    tic = time()
    # target = './submits/log_CE_Net/'
    # target = '/data/guoxuejun/yangjialin/CE-Net/submits/log_CE_Net/'
    #target = '/data/guoxuejun/yangjialin/CE-Net/submits/log_CE_attention_Net1/'
    target = './submits/log_RCFSNet_20_road/'
    if not os.path.exists(target):
        os.mkdir(target)
    #gt_root = '/data/guoxuejun/yangjialin/CE-Net/dataset/ROAD/test/masks'
    gt_root = './dataset/ROAD/test/masks'
    ...
    for i, name in enumerate(val):
        ...
        ground_truth_path = os.path.join(gt_root, name.split('_')[0] + '_mask.png')
        ...
        acc, sen, iou,pre,f1 = accuracy(predi_mask[:, :, 0], gt)
